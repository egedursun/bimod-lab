...
class MetaIntegrationTeam(models.Model):
    meta_integration_category = models.ForeignKey(
    ...
    meta_integration_name = models.CharField(
        max_length=1000,
        null=False,
        blank=False
    )
    ...
    meta_integration_team_image = models.ImageField(
        upload_to='meta_integration_team_images/%Y/%m/%d/',
        null=True,
        blank=True
    )

    integration_assistants = models.ManyToManyField(
