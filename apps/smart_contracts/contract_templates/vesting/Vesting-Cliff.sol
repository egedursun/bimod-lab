...
contract CliffVesting {
    ...
    constructor(IERC20 _token, address _beneficiary, uint256 _startTime, uint256 _cliffDuration, uint256 _duration, uint256 _totalTokens) {
    ...
    function _vestedAmount() internal view returns (uint256) {
        if (block.timestamp >= startTime + duration) {
            return totalTokens;
        } else if (block.timestamp >= startTime + cliffDuration) {
