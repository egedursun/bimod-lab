...
def remove_vector_from_index_on_sql_database_delete(
    sender,
    instance,
    **kwargs
):
    try:
        ...
        if os.path.exists(index_path):
            ...
            logger.warning(f"Index file not found for SQLDatabaseConnection with ID {instance.id}.")

        for vector_data_instance in vector_data_instances:
            vector_data_instance.delete()

    except SQLSchemaChunkVectorData.DoesNotExist:
        logger.warning(f"Vector data not found for SQLDatabaseConnection with ID {instance.id}.")
