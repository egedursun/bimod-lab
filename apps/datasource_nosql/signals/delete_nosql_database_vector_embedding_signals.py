...
from apps.datasource_nosql.utils import VECTOR_INDEX_PATH_NOSQL_SCHEMAS

logger = logging.getLogger(__name__)


@receiver(pre_delete, sender=NoSQLDatabaseConnection)
def remove_vector_from_index_on_nosql_database_delete(sender, instance, **kwargs):
    try:
        ...
        if os.path.exists(index_path):
            ...
            print(f"Removed vector data for NoSQLDatabaseConnection with ID {instance.id} from index.")
        ...
        print(
            f"No NoSQLSchemaChunkVectorData found for NoSQLDatabaseConnection with ID {instance.id}.")
