...
def remove_vector_from_index_on_integration_delete(sender, instance, **kwargs):
    try:
        vector_data_instance = IntegrationVectorData.objects.get(integration_assistant=instance)
        ...
        if os.path.exists(index_path):
            ...
            print(f"Removed vector data for AssistantIntegration with ID {instance.id} from index.")
        else:
            print(f"Index path {index_path} does not exist.")

        vector_data_instance.delete()
    except IntegrationVectorData.DoesNotExist:
        print(f"No IntegrationVectorData found for AssistantIntegration with ID {instance.id}.")
