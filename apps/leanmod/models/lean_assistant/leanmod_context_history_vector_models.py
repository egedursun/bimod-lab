...
class LeanModOldChatMessagesVectorData(models.Model):
    leanmod_chat_message = models.ForeignKey('multimodal_chat.MultimodalLeanChatMessage', on_delete=models.CASCADE,
                                             related_name='leanmod_chat_message_vector_data')
    ...
    def __str__(self):
        return str(self.leanmod_chat_message.multimodal_lean_chat.lean_assistant.id) + " - " + str(
            self.leanmod_chat_message.multimodal_lean_chat.id) + " - " + str(
            self.leanmod_chat_message.id) + " - " + self.created_at.strftime(
            "%Y-%m-%d %H:%M:%S")
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['leanmod_chat_message']),
            models.Index(fields=['created_at']),
            models.Index(fields=['updated_at']),
        ]

    def _get_index_path(self):
        leanmod_assistant_chat_id = self.leanmod_chat_message.multimodal_lean_chat.id
        return os.path.join(VECTOR_INDEX_PATH_LEANMOD_CHAT_MESSAGES,
                            f'leanmod_chat_index_{leanmod_assistant_chat_id}.index')
    ...
    def save(self, *args, **kwargs):
        ...
        if self.leanmod_chat_message.multimodal_lean_chat.lean_assistant.context_overflow_strategy == ContextManagementStrategyNames.VECTORIZE:
            # Vectorize the messages if it is within the context management strategy of the LeanMod Assistant object
            ...
            raw_data = {
                ...
                "sent_at": self.leanmod_chat_message.sent_at.strftime("%Y-%m-%d %H:%M:%S"),
            }
    ...
    def _generate_embedding(self, raw_data):
        ...
        c = OpenAIGPTClientManager.get_naked_client(
            llm_model=self.leanmod_chat_message.multimodal_lean_chat.lean_assistant.llm_model)
