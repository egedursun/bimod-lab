...
class OrchestrationView_ConnectReactantAssistantToOrchestration(LoginRequiredMixin, TemplateView):
    def get_context_data(self, **kwargs):
        ...
        orchestration_maestros = Maestro.objects.filter(
            organization__in=user_orgs
        ).all()
        ...
        context["existing_connections"] = OrchestrationReactantAssistantConnection.objects.filter(
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not UserPermissionManager.is_authorized(
            ...
            messages.error(
                self.request,
                "You do not have permission to connect reactant assistants to an orchestration."
            )
        ...
        maestro_instance = Maestro.objects.get(
            id=maestro_instance_id
        )
