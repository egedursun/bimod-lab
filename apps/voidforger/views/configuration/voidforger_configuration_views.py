...
class VoidForgerView_Configuration(LoginRequiredMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not UserPermissionManager.is_authorized(
            ...
            messages.error(
                self.request,
                "You do not have permission to update VoidForger configurations."
            )
            return redirect('voidforger:configuration')
        ##############################

        config: VoidForger = VoidForger.objects.filter(
            user=request.user
        ).first()
        ...
        if config:
            try:
                ...
                llm_model = LLMCore.objects.filter(
                    id__in=llm_model_id
                ).first()
                ...
                if response_language not in [
                    x for x, y in AGENT_SPEECH_LANGUAGES
                ]:
