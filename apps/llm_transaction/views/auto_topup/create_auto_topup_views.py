...
class Transactions_AutoTopUpCreate(LoginRequiredMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        org_id = request.POST.get(
            'organization_id'
        )

        on_balance_threshold_trigger = request.POST.get(
            'on_balance_threshold_trigger'
        )

        on_interval_by_days_trigger = request.POST.get(
        ...
        if on_balance_threshold_trigger:
            balance_lower_trigger_threshold_value = request.POST.get(
                'balance_lower_trigger_threshold_value'
            )

            addition_on_balance_threshold_trigger = request.POST.get(
        ...
        if on_interval_by_days_trigger:
            regular_by_days_interval = request.POST.get(
                'regular_by_days_interval'
            )

            addition_on_interval_by_days_trigger = request.POST.get(
                'addition_on_interval_by_days_trigger'
            )

            date_of_last_auto_top_up = timezone.now()

        monthly_hard_limit_auto_addition_amount = request.POST.get(
            'monthly_hard_limit_auto_addition_amount',
            100_000
        )
        ...
        try:
            top_up_model = AutoBalanceTopUpModel.objects.create(
                ...
                addition_on_balance_threshold_trigger=round(
                    float(
                        addition_on_balance_threshold_trigger
                    ),
                    2
                ) if addition_on_balance_threshold_trigger else None,
                regular_by_days_interval=regular_by_days_interval,
                addition_on_interval_by_days_trigger=round(
                    float(
                        addition_on_interval_by_days_trigger
                    ),
                    2
                ) if addition_on_interval_by_days_trigger else None,
