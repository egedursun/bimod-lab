...
class TransactionInvoice(models.Model):
    ...
    payment_method = models.CharField(
        max_length=50,
        choices=ACCEPTED_METHODS_OF_PAYMENT,
        default="credit-card"
    )
    ...
    barcode_image = models.ImageField(
        upload_to='invoice_barcodes/%Y/%m/%d/',
        null=True,
        blank=True
    )

    transaction_paper = models.FileField(
        upload_to='transaction_papers/%Y/%m/%d/',
        null=True,
        blank=True
    )
    ...
    def save(self, force_insert=False, force_update=False, using=None, update_fields=None):
        if not self.pk:
            ...
            filename, invoice_paper = invoice_paper_generator(
                self,
                barcode
            )

            if invoice_paper:
                self.transaction_paper.save(
                    filename,
                    invoice_paper,
                    save=False
                )
