...
class LLMTransaction(models.Model):
    ...
    internal_service_cost = models.DecimalField(
        max_digits=10,
        decimal_places=6,
        null=True,
        blank=True
    ...
    def save(self, *args, **kwargs):
        ...
        try:
            if (
                self.transaction_context_content and
                self.is_tool_cost is False
            ):
                self.number_of_tokens = process_and_calculate_number_of_billable_tokens(
                    self.encoding_engine,
                    self.transaction_context_content
                )

                self.llm_cost = calculate_total_llm_model_costs(
                    self.model.model_name,
                    self.number_of_tokens
                )

                self.internal_service_cost = calculate_service_costs_of_platform(
                    self.llm_cost
                )

                self.tax_cost = calculate_value_added_tax(
                    self.internal_service_cost
                )

                self.total_billable_cost = calculate_final_billable_cost(
                    self.internal_service_cost,
                    self.tax_cost
                )

                self.total_cost = calculate_final_cost_total(
                    self.llm_cost,
        ...
        try:
            if self.is_tool_cost:
                tool_cost = float(
                    TOOL_NAME_TO_COST_MAP[self.transaction_source]
                ) or 0.000000

                self.internal_service_cost = calculate_service_costs_of_platform(
                    self.llm_cost,
                    tool_service_fee_absolute_rate=tool_cost
                )
