...
def unregister_periodic_task(
    sender,
    instance,
    **kwargs
):
    PeriodicTask.objects.filter(
        name=f'ScheduledJob-{instance.id}'
    ).delete()


@receiver(post_delete, sender=OrchestrationScheduledJob)
def unregister_periodic_task_orchestration(
    sender,
    instance,
    **kwargs
):
    PeriodicTask.objects.filter(
        name=f'OrchestrationScheduledJob-{instance.id}'
    ).delete()


@receiver(post_delete, sender=LeanModScheduledJob)
def unregister_periodic_task_leanmod(
    sender,
    instance,
    **kwargs
):
    PeriodicTask.objects.filter(
        name=f'LeanModScheduledJob-{instance.id}'
    ).delete()
