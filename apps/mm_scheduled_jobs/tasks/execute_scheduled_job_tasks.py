...
def execute_scheduled_job(scheduled_job_id):
    ...
    new_instance = ScheduledJobInstance.objects.create(scheduled_job=job,
...
def execute_orchestration_scheduled_job(scheduled_job_id):
    ...
    job = OrchestrationScheduledJob.objects.get(id=scheduled_job_id)
    if job.current_run_count > job.maximum_runs:
        job.delete()
        return

    new_instance = OrchestrationScheduledJobInstance.objects.create(scheduled_job=job,
                                                                    status=ScheduledJobInstanceStatusesNames.PENDING)
    job.scheduled_job_instances.add(new_instance)
    job.save()
    ...
    try:
        ...
        query = OrchestrationQuery.objects.create(
            maestro=new_instance.scheduled_job.maestro, query_text=instruction_feed, created_by_user=new_instance.scheduled_job.created_by_user,
