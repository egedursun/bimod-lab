...
class MediaView_ItemAIDescription(LoginRequiredMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            media_item = DataSourceMediaStorageItem.objects.get(
                id=item_id
            )

            xc_type = self.decode_media_item_type(media_item.media_file_type)

            txts, _, _ = run_query_media_manager(
                chat_id=None,
                c_id=media_item.storage_base.id,
                manager_file_type=xc_type,
                f_uris=[media_item.full_file_path],
