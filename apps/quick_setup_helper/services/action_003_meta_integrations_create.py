...
from apps.meta_integrations.utils import (
    META_INTEGRATION_ORCHESTRATOR_STANDARD_INSTRUCTIONS,
...
from apps.quick_setup_helper.utils import (
    ASSISTANT_INTEGRATION_SEARCH_RELATED_LIMIT
)
    ...
    try:
        ...
        for use_case_query in response__assistant_use_cases_list:
            ...
            for search_result in search_results:
                integration_raw_data = search_result.get(
                    "data"
                )

                integration_teams = integration_raw_data.get(
            ...
            total_created_assistants.extend(
                integrated_assistants
            )
            total_created_leanmods.append(
                integrated_leanmods
            )

            total_created_orchestrators.append(
                integrated_orchestrators
            )
...
def integrate_meta_integration_team_naked(
    user: User,
    ...
    try:
        meta_integration_data: MetaIntegrationTeam = MetaIntegrationTeam.objects.get(
            meta_integration_name=team_name
        )
        ...
        try:
            ...
            orchestration_maestro.workers.set(
        ...
        try:
            ...
            created_lean_assistant.expert_networks.set(
                [
                    expert_network
                ]
            )
