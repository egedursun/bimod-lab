...
class PermissionView_PermissionList(LoginRequiredMixin, TemplateView):
    def get_context_data(self, **kwargs):
        ...
        org_users_permissions = {
            org: {
                "users": {
                    user: user.permissions.all() for user in org.users.all()
                }
    ...
    def post(self, request, *args, **kwargs):
        ...
        for permission in user.permissions.all():
            ...
            if (
                str(permission.id) in permissions_data and
                ...
                str(permission.id) not in permissions_data and
        ...
        for permission_id in delete_requests:
            ...
            deletion_names.append(
                permission.get_permission_type_code()
            )
        ...
        if user.is_superuser:
            UserPermission.objects.get_or_create(
                user=user,
                permission_type=modify_permissions
            )

            UserPermission.objects.get_or_create(
                user=user,
                permission_type=list_permissions
            )

            modify_object = UserPermission.objects.get(
                user=user,
                permission_type=modify_permissions
            )

            list_object = UserPermission.objects.get(
                user=user,
                permission_type=list_permissions
            )

            if (
                not modify_object.is_active or
                not list_object.is_active or
                modify_permissions in deletion_names
                or list_permissions in deletion_names
            ):
                messages.warning(
                    request,
                    'You have removed your permission rights as an administrator, '
