...
class AuxiliaryInteractionsManager:
    ...
    @staticmethod
    def calculate_total_messages_per_organizations(orgs, n_days):
        result = {}
        for org in orgs:
    ...
    @staticmethod
    def calculate_total_request_count_per_exported_assistants(exp_agents, n_days):
        result = {}
        for exp_a in exp_agents:
            reqs = RequestLog.objects.filter(
                export_assistant=exp_a, timestamp__gte=timezone.now() - timezone.timedelta(days=n_days)
            )
            total = reqs.count()
            if total > 0:
