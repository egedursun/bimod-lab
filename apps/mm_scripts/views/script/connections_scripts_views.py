...
class CustomScriptView_Connections(LoginRequiredMixin, TemplateView):
    def get_context_data(self, **kwargs):
        ...
        ext_script_refs = CustomScriptReference.objects.filter(
            assistant__organization__in=conn_orgs
        ).exclude(
            custom_script__created_by_user__in=users_of_conn_orgs
        )

        agents = Assistant.objects.filter(
            organization__in=conn_orgs
        ).select_related(
            'organization'
        )
        ...
        ext_script_refs_map = {
            assistant.id: set(
                reference for reference in ext_script_refs if reference.assistant.id == assistant.id
            )
            for assistant in agents
        }
        ...
        context.update(
            {
                'connected_organizations': conn_orgs,
                'scripts': scripts,
                'assistants': agents,
                'assistant_script_map': agent_script_map,
                'external_script_references_map': ext_script_refs_map
            }
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if action == 'add' and script_id:
                ...
                messages.success(
                    request,
                    f"Script '{custom_script.name}' assigned to assistant '{agent.name}'."
                )
                ...
                if ref_id:
                    ref = CustomScriptReference.objects.get(
                        id=ref_id
                    )
