...
class CustomScriptView_Store(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        scripts_list = CustomScript.objects.filter(
            is_public=True
        )
        ...
        script_agent_map = {
            script.id: Assistant.objects.exclude(
                customscriptreference__custom_script=script
            )
        ...
        context['public_scripts'] = CustomScript.objects.filter(
            is_public=True
        ).count()

        context['private_scripts'] = CustomScript.objects.filter(
            is_public=False
        ).count()
