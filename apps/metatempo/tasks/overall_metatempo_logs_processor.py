...
def generate_period_overall_metatempo_logs():
    ...
    try:
        ...
        for connection in connections:
            ...
            last_tracked_log = MetaTempoProjectOverallLog.objects.filter(
            ...
            if not last_tracked_log:
                ...
                last_tracked_log = last_tracked_log.order_by(
                    '-created_at'
                ).first()
            ...
            if last_tracked_log:
                ...
                if time_since_last_log_creation > timedelta(
                    hours=delta_value_hours
                ):
                    logger.info(f"Creating a new log as the last log is older than the interval for "
                ...
                logger.info(f"Creating a new log as there is no log for connection {connection.id}")
                pass
            ...
            try:
                ...
                logger.info(
                    f"[interpret_overall_logs] Created LLMTransaction for MetaTempo [AUTO] Overall Analysis Log.")
