...
from apps.core.codebase.handler_methods.creation_handler import (
    create_classes_helper
)

from apps.core.codebase.handler_methods.removal_handler import (
    delete_weaviate_class_helper
)

from apps.core.codebase.handler_methods.removal_handler_repo import (
    delete_repository_helper
)
...
class WeaviateExecutor:
    ...
    def delete_weaviate_classes(self, class_name: str):
        try:
            ...
            output = delete_weaviate_class_helper(
                executor=self,
        ...
        try:
            for root, dirs, files in os.walk(cloned_repository_path):
                for file in files:
                    ...
                    formatted_content += self.assign_line_numbers_and_filename_to_line(
                        str(file_path),
                        content_lines
                    )
    ...
    def chunk_repository(
        ...
        try:
            chunks = split_repository_into_chunks(
                connection_id,
                document
            )
    ...
    def embed_repository(
        ...
        try:
            doc_id, doc_uuid, error = embed_repository_data(
    ...
    def embed_repository_chunks(
        ...
        try:
            errors = embed_repository_chunks(
                executor_params=executor_params,
                chunks=chunks,
                path=path,
    ...
    def search_hybrid(
        ...
        try:
            ...
            tx = LLMTransaction(
                ...
                llm_cost=InternalServiceCosts.CodeBaseExecutor.COST,
                transaction_type=ChatRoles.SYSTEM,
                transaction_source=LLMTransactionSourcesTypesNames.CODE_BASE_SEARCH,
                is_tool_cost=True
            )
