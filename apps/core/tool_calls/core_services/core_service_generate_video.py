...
def run_generate_video(connection_id: int,
    ...
    try:
        if video_generator_action_type == VideoGenerationActionTypes.TEXT_TO_VIDEO_WITH_LOOP_AND_ASPECT_RATIO:
            ...
            response = xc.text_to_video(query=video_generator_query, loop=True, with_aspect_ratio=False)
        elif video_generator_action_type == VideoGenerationActionTypes.TEXT_TO_VIDEO_WITH_ASPECT_RATIO:
            response = xc.text_to_video(query=video_generator_query, loop=False, with_aspect_ratio=True,
                                        aspect_ratio=aspect_ratio)
        elif video_generator_action_type == VideoGenerationActionTypes.TEXT_TO_VIDEO:
            response = xc.text_to_video(query=video_generator_query, loop=False, with_aspect_ratio=False)
        elif video_generator_action_type == VideoGenerationActionTypes.TEXT_AND_IMAGE_TO_VIDEO_WITH_LOOP_AND_WITH_START_FRAME:
            response = xc.text_and_image_to_video(query=video_generator_query, loop=True,
            ...
            response = xc.text_and_image_to_video(query=video_generator_query, loop=False,
                                                  frame_type=VideoGeneratorFrameTypes.START,
                                                  start_frame_url=start_frame_url)
