...
def run_query_execute_smart_contract_generation_query(user: User, llm_model: LLMCore, wallet_id: int, nickname: str,
                                                      description: str, category: str, contract_template: str,
                                                      creation_prompt: str, maximum_gas_limit: int,
                                                      gas_price_gwei: int):
    try:
        blockchain_wallet = BlockchainWalletConnection.objects.get(id=wallet_id)
        ...
        xc = SmartContractsExecutionManager(smart_contract_object=smart_contract_object, llm_model=llm_model)
        success, error = xc.generate_contract_and_save_content(previous_mistakes_prompt=None)
