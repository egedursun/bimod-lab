...
def run_query_execute_metatempo(c_id: int, action: str, query: str):
    try:
        connection = MetaTempoAssistantConnection.objects.get(id=c_id)
        xc = MetaTempoExecutionManager(metatempo_connection_id=connection.metatempo_instance.id)
        if action == "QUERY":
            output, error = xc.answer_logs_question(user_query=query)
            if error:
