...
def execute_semantor_search_query(user, llm_model, query):
    ...
    try:
        ...
        semantor_config, _ = SemantorConfiguration.objects.get_or_create(user=user)
        semantor_config: SemantorConfiguration
        if semantor_config.is_local_network_active:
            local_network_output = xc.search_assistants(query=query)
        else:
        ...
        if semantor_config.is_global_network_active:
            global_network_output = xc.search_integrations(query=query)
        else:
