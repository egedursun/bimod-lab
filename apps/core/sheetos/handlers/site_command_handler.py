...
#
#  Project: Bimod.io™
#  File: site_command_handler.py
#  Last Modified: 2024-12-09 21:51:32
#  Author: Ege Dogan Dursun (Co-Founder & Chief Executive Officer / CEO @ BMD™ Autonomous Holdings)
#  Created: 2024-12-09 21:51:32
#
...
def handle_site_command(xc, command: str):
    ...
    try:
        tx = LLMTransaction.objects.create(
            organization=xc.sheetos_document.organization,
            model=xc.copilot_llm,
            responsible_user=xc.sheetos_document.created_by_user,
        ...
        logger.error(f"[handle_site_command] Error creating LLMTransaction for user command: {command}. Error: {e}")
        pass

    output, error = None, None

    system_prompt = build_site_command_system_prompt(
        xc=xc,
        user_query=command
    )
    ...
    try:
        ...
        logger.error(f"[handle_site_command] Error creating LLMTransaction for system prompt. Error: {e}")
        pass
    ...
    try:
        ...
        logger.info(f"[handle_site_command] Generated SITE AI response.")
        ...
        try:
            ...
            logger.error(f"[handle_site_command] Error creating LLMTransaction for AI response. Error: {e}")
            pass

    except Exception as e:
        error = f"[handle_site_command] Error executing VECTOR command: {command}. Error: {e}"
        logger.error(error)
    ...
    while (
        ...
        try:
            ...
            logger.error(f"[handle_site_command] Error executing VECTOR command: {command}. Error: {e}")
            error = f"[handle_site_command] Error executing VECTOR command: {command}. Error: {e}"
...
def _handle_tool_website_data_search(
    ...
    output = run_query_search_website_data(
        connection_id=c_id,
