...
def handle_web_command(xc, command: str) -> str:
    ...
    system_prompt = build_web_command_system_prompt(xc=xc, user_query=command)
    client = xc.naked_c
    ...
    while len(find_tool_call_from_json(choice_message_content)) > 0 and (tool_counter < SHEETOS_TOOL_CALL_MAXIMUM_ATTEMPTS):
        tool_counter += 1
