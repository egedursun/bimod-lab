...
def build_sheetos_action__sql_prompt(user_query: str):
    return f"""
        ...
        **YOUR GOAL:** The user picked the SQL Command action. You MUST check the user's query and understand what
        ...
        you found in the database, you must generate the data in plain CSV-format and in a state and content requested
        by the user's query. If you have the information specified in your own internal knowledge, you don't need to
        perform a database search. But be careful in not providing false information (unless specifically requested by
        the user). Always share your results in plain CSV format.
