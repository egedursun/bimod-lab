...
def build_sheetos_code_base_data_source_prompt(assistant: Assistant):
    ...
    response_prompt += """
            ...
            while generating/modifying coherent CSV data within the spreadsheet document. If this part is EMPTY, it
            means that the user has not provided any Code Base Storage Connections (yet), so neglect this part.
            ...
            - IF the user requests data generation about a certain topic, person, issue, or anything that is not
            within your own knowledge, instead of telling the user 'YOU DONT KNOW', 'USE' the storages to see if you
            can find something relevant to the question and **NEVER** deliver your response in natural language,
            always output plain CSV-formatted data or if you are going to use a tool; output plain JSON without any
            additional characters, specifiers, explanatory text, comments, etc.
