...
#  Last Modified: 2024-10-16 01:33:09
...
def build_sheetos_sql_data_source_prompt(assistant: Assistant):
    ...
    response_prompt += """
        ...
        #### **NOTE**: These are the SQL Database Connections that you have access. Keep these in mind while generating
        CSV data for the user within the context of the spreadsheet document. Custom queries are also provided for
        each SQL Connection, which you can use to fetch data from the respective database or if you have the write
        permissions, you can use them to write data as well. If this part is EMPTY, it means that the user has not
        provided any SQL Database Connections (yet), so neglect this part.
