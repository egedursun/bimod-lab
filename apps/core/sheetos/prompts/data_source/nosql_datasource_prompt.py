...
def build_sheetos_nosql_data_source_prompt(assistant: Assistant):
    ...
    response_prompt += """
        ...
        generating CSV content for to user. Custom queries are also provided for each NoSQL Connection, which you can
        use to fetch  data from the respective database or if you have the write permissions, you can use them to write d
        ata as well. If this part is EMPTY, it means that the user has not provided any NoSQL Database Connections (yet), so
        neglect this part.
