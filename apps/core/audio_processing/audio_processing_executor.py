    ...
    def convert_audio_to_text(self, audio_file_path: str):
        ...
        try:
            tx = LLMTransaction(
                ...
                encoding_engine=GPT_DEFAULT_ENCODING_ENGINE, llm_cost=InternalServiceCosts.AudioProcessingSTT.COST,
                transaction_type=ChatRoles.SYSTEM,
                transaction_source=LLMTransactionSourcesTypesNames.AUDIO_PROCESSING_STT, is_tool_cost=True
            )
            tx.save()
    ...
    def convert_text_to_audio_message(self, message):
        ...
        try:
            tx = LLMTransaction(
                organization=self.chat.assistant.organization, model=self.chat.assistant.llm_model,
                responsible_user=self.chat.user, responsible_assistant=self.chat.assistant,
                encoding_engine=GPT_DEFAULT_ENCODING_ENGINE, llm_cost=InternalServiceCosts.AudioProcessingTTS.COST,
                transaction_type=ChatRoles.SYSTEM,
                transaction_source=LLMTransactionSourcesTypesNames.AUDIO_PROCESSING_TTS, is_tool_cost=True
            )
            tx.save()
        except Exception as e:
            logger.error(f"Error while saving LLM Transaction: {str(e)}")
