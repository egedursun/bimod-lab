...
def _metakanban_create_label_query(
    board_id: int,
    action_content: dict
):
...
def _metakanban_update_label_query(
    board_id: int,
    action_content: dict
):
    board = MetaKanbanBoard.objects.get(
    ...
    try:
        label = MetaKanbanTaskLabel.objects.get(
            id=action_content.get("label_id")
        )
...
def _metakanban_delete_label_query(
    board_id: int,
    action_content: dict
):
    board = MetaKanbanBoard.objects.get(
...
def _metakanban_create_column_query(
    board_id: int,
    action_content: dict
):
    board = MetaKanbanBoard.objects.get(
    ...
    def reorder_columns(b_id):
        cols = MetaKanbanStatusColumn.objects.filter(
            board_id=b_id
    ...
    try:
        ...
        with transaction.atomic():
            MetaKanbanStatusColumn.objects.filter(
                ...
                position_id=models.F("position_id") + 1
            )
...
def _metakanban_update_column_query(
    board_id: int,
    action_content: dict
):
    board = MetaKanbanBoard.objects.get(
    ...
    updatable_column = MetaKanbanStatusColumn.objects.get(
        id=updatable_column_id
    ).first()
...
def _metakanban_delete_column_query(
    board_id: int,
    action_content: dict
):
    board = MetaKanbanBoard.objects.get(
    ...
    column = MetaKanbanStatusColumn.objects.get(
        id=column_id
    ).first()
    ...
    try:
        column = MetaKanbanStatusColumn.objects.get(
            id=action_content.get("column_id")
        )
...
def _metakanban_create_task_query(
    board_id: int,
    action_content: dict
):
    board = MetaKanbanBoard.objects.get(
...
def _metakanban_update_task_query(
    board_id: int,
    action_content: dict
):
    board = MetaKanbanBoard.objects.get(
    ...
    try:
        task = MetaKanbanTask.objects.get(
            id=action_content.get("task_id")
        )

        task.status_column_id = action_content.get(
            "status_column_id",
            task.status_column_id
        )

        task.title = action_content.get(
            "title",
            task.title
        )

        task.description = action_content.get(
            "description", task.description
        )

        task.priority = action_content.get(
            "priority", task.priority
        )

        task.due_date = action_content.get(
            "due_date", task.due_date
        )

        task.task_url = action_content.get(
            "task_url", task.task_url
        )

        task.labels.set(
            action_content.get(
                "label_ids", task.labels.all()
            )
        )

        task.assignees.set(
            action_content.get(
                "assignee_ids", task.assignees.all()
            )
        )
        task.save()
    except Exception as e:
...
def _metakanban_delete_task_query(
    board_id: int,
    action_content: dict
):
    board = MetaKanbanBoard.objects.get(
...
def _metakanban_assign_task_query(
    board_id: int,
    action_content: dict
):
    board = MetaKanbanBoard.objects.get(
...
def _metakanban_move_task_query(
    board_id: int,
    action_content: dict
):
    board = MetaKanbanBoard.objects.get(
