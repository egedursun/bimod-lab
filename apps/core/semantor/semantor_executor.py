...
class SemantorVectorSearchExecutionManager:
    def __init__(
        ...
        if os.path.exists(self.assistants_index_path):
            self.assistants_index = faiss.read_index(
                self.assistants_index_path
            )

        else:
            self.assistants_index = faiss.IndexIDMap(faiss.IndexFlatL2(
                self.vector_dim)
            )

            faiss.write_index(
                self.assistants_index,
                self.assistants_index_path
            )

        if os.path.exists(self.leanmod_assistants_index_path):
            self.leanmod_assistants_index = faiss.read_index(
                self.leanmod_assistants_index_path
            )

        else:
            self.leanmod_assistants_index = faiss.IndexIDMap(
                faiss.IndexFlatL2(
                    self.vector_dim
                )
            )
            faiss.write_index(
                self.leanmod_assistants_index,
                self.leanmod_assistants_index_path
            )

        if os.path.exists(self.integrations_index_path):
            self.integrations_index = faiss.read_index(
                self.integrations_index_path
            )

        else:
            self.integrations_index = faiss.IndexIDMap(
                faiss.IndexFlatL2(
                    self.vector_dim
                )
            )
    ...
    def search_assistants(
        self, query: str,
        n_results: int = SEMANTOR_DEFAULT_SEARCH_RESULTS_ASSISTANTS
    ) -> List[Dict]:
        ...
        for item_id, distance in zip(ids[0], distances[0]):
            ...
            try:
                instance = AssistantVectorData.objects.get(
                    id=item_id
                )
    ...
    def search_integrations(
        self,
        query: str,
        n_results: int = SEMANTOR_DEFAULT_SEARCH_RESULTS_INTEGRATIONS
    ) -> List[Dict]:
        ...
        distances, ids = self.integrations_index.search(
            query_vector,
            n_results
        )
        ...
        for item_id, distance in zip(ids[0], distances[0]):
            ...
            try:
                instance = IntegrationVectorData.objects.get(
                    id=item_id
                )
    ...
    def consult_semantor_by_query(
        ...
        if is_local:
            expert_agent_vector: AssistantVectorData = AssistantVectorData.objects.get(
            ...
            try:
                new_chat_object = MultimodalChat.objects.create(
                    ...
                    user=self.llm_model.created_by_user,
                    chat_source=SourcesForMultimodalChatsNames.ORCHESTRATION,
                    is_archived=False,
                    created_by_user_id=self.llm_model.created_by_user.id
                )
            ...
            temporary_assistant = Assistant.objects.create(
                id=0,
            ...
            try:
                temporary_chat_object = MultimodalChat.objects.create(
                    organization=temporary_assistant.organization,
            ...
            try:
                semantor_config: SemantorConfiguration = SemantorConfiguration.objects.get(
                    user=self.user
                )

                if semantor_config is None:
                    semantor_config = SemantorConfiguration.objects.create(
                        user=self.user
                    )
            ...
            if temporary_data_source_and_tool_access is True:
                temporary_sources = self._gather_temporary_sources(
                    temporary_assistant
                )
            ...
            logger.info(
                f"Created temporary chat message object for Semantor network consultation response: {final_response}")
    ...
    def search_leanmod_assistants(
        self,
        query: str,
        n_results: int = SEMANTOR_DEFAULT_SEARCH_RESULTS_LEANMOD_ASSISTANTS
    ) -> List[Dict]:
        ...
        for item_id, distance in zip(
            ...
            try:
                instance = LeanModVectorData.objects.get(
                    id=item_id
                )
    ...
    def consult_leanmod_oracle_by_query(
        ...
        leanmod_oracle_vector: LeanModVectorData = LeanModVectorData.objects.get(
            id=consultation_object_id
        )

        lean_assistant: LeanAssistant = leanmod_oracle_vector.leanmod_assistant

        structured_order = build_voidforger_to_leanmod_assistant_instructions_prompt(
            query_text=query
        )
        ...
        try:
            llm_client: OpenAIGPTLeanClientManager = OpenAIGPTLeanClientManager(
    ...
    def _gather_temporary_sources(
        ...
        reference_assistants = Assistant.objects.filter(
            organization__in=[
                temporary_assistant.organization
            ]
        )
        ...
        for assistant in reference_assistants:
            ...
            for sql_db in assistant__data_source__sql_dbs:
                ...
                if (
                    sql_db.host not in sql_recorded_hosts and
                    sql_db.database_name not in sql_recorded_db_names
                ):
            ...
            for nosql_db in assistant__data_source__nosql_dbs:
                ...
                if (
                    nosql_db.host not in nosql_recorded_hosts and
                    nosql_db.bucket_name not in nosql_recorded_bucket_names
                ):
            ...
            for website_storage in assistant__data_source__website_storages:
                website_storage: DataSourceWebsiteStorageConnection

                if recorded_website_storage_count == 5:
                    break

                if website_storage.id not in recorded_website_storage_ids:
                    data_source__website_storages.append(website_storage)
                    recorded_website_storage_ids.append(website_storage.id)
                    recorded_website_storage_count += 1

            ######

            source_references["data_sources"]["website_storages"] = data_source__website_storages
            pass
