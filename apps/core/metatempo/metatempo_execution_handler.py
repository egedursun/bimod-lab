...
class MetaTempoExecutionManager:
    ...
    def _consult_ai(self, action_type: str, interpretation_query: str = None, log_screenshot_data=None,
        ...
        if action_type == MetaTempoExecutionActionTypesNames.INTERPRET_AND_SAVE_LOG_SNAPSHOT:
            ...
            try:
                tx = LLMTransaction.objects.create(
                    organization=self.metatempo_connection.board.project.organization, model=self.llm_model,
                    responsible_user=self.metatempo_connection.board.created_by_user, responsible_assistant=None,
                    encoding_engine=GPT_DEFAULT_ENCODING_ENGINE, transaction_context_content=str(action_specific_system_prompt),
                    llm_cost=0, internal_service_cost=0, tax_cost=0, total_cost=0, total_billable_cost=0,
                    transaction_type=ChatRoles.SYSTEM, transaction_source=LLMTransactionSourcesTypesNames.METATEMPO
                )
            ...
            context_messages_history.append(
                {"role": "user", "content": [
                    {"type": "text", "text": interpretation_query},
                    {"type": "image_url", "image_url": {"url": formatted_uri}}
                ]}
            )
            ...
            action_specific_system_prompt = build_daily_logs_interpretation_prompt(
                connection=self.metatempo_connection, batched_logs=batched_logs)
            ...
            try:
                tx = LLMTransaction.objects.create(
                    organization=self.metatempo_connection.board.project.organization, model=self.llm_model,
                    responsible_user=self.metatempo_connection.created_by_user, responsible_assistant=None,
                    encoding_engine=GPT_DEFAULT_ENCODING_ENGINE, transaction_context_content=str(interpretation_query),
                    llm_cost=0, internal_service_cost=0, tax_cost=0, total_cost=0, total_billable_cost=0,
                    transaction_type=ChatRoles.USER, transaction_source=LLMTransactionSourcesTypesNames.METATEMPO
                )
        ...
        try:
            llm_output = self.c.chat.completions.create(
                model=self.llm_model.model_name, messages=context_messages_history,
        ...
        try:
            tx = LLMTransaction.objects.create(
                ...
                llm_cost=0, internal_service_cost=0, tax_cost=0, total_cost=0, total_billable_cost=0,
                transaction_type=ChatRoles.ASSISTANT, transaction_source=LLMTransactionSourcesTypesNames.METATEMPO
            )
            logger.info(
    ...
    def interpret_and_save_daily_logs_batch(self, users: list):
        ...
        try:
            for user in users:
                ...
                log_records = MetaTempoMemberLog.objects.filter(user=user, timestamp__gte=timestamp_minus_24_hours)
                log_outputs, error = self._interpret_and_save_daily_logs(context_user=user, log_records=log_records)
                if error:
                ...
                try:
                    tx = LLMTransaction(
                        organization=self.metatempo_connection.board.project.organization, model=self.metatempo_connection.board.llm_model,
                        responsible_user=self.metatempo_connection.board.created_by_user, responsible_assistant=None,
    ...
    def _interpret_and_save_daily_logs(self, context_user: User, log_records: list):
        response_json_string, error = self._consult_ai(
    ...
    def interpret_overall_logs(self):
        ...
        timestamp_minus_interval = timezone.now() - timedelta(hours=delta_value_hours)
        log_records = MetaTempoMemberLogDaily.objects.filter(datestamp__gte=timestamp_minus_interval)
    ...
    def answer_logs_question(self, user_query: str):
        ...
        response_text, error = self._consult_ai(action_type=MetaTempoExecutionActionTypesNames.ASK_LOGS_QUESTION,
