...
class SmartContractsExecutionManager:
    ...
    def _ai_generate_contract_code(self, messages: list, previous_mistakes_prompt):
        ...
        try:
            ...
            tx = LLMTransaction.objects.create(
                organization=self.wallet.organization, model=self.llm_model,
                responsible_user=self.wallet.created_by_user, responsible_assistant=None,
            ...
            llm_response = self.c.chat.completions.create(
                model=self.llm_model.model_name, messages=conversation_messages,
    ...
    def _create_contract(self, previous_mistakes_prompt):
        ...
        for i in range(0, n_iterations, 1):
            ...
            tx = LLMTransaction(
                organization=self.wallet.organization, model=self.llm_model,
                responsible_user=self.wallet.created_by_user, responsible_assistant=None,
                encoding_engine=GPT_DEFAULT_ENCODING_ENGINE, llm_cost=InternalServiceCosts.SmartContractCreation.COST,
                transaction_type=ChatRoles.SYSTEM,
                transaction_source=LLMTransactionSourcesTypesNames.SMART_CONTRACT_CREATION, is_tool_cost=True
            )
            tx.save()
        ...
        if nlp_feed_prompt is not None and nlp_feed_prompt != "":
            try:
                ...
                structured_messages = [{"role": "system", "content": nlp_feed_prompt}]
    ...
    def deploy_contract(contract_obj):
        ...
        try:
            ...
            transaction = contract.constructor(**contract_args).build_transaction({
                ...
                'gasPrice': web3.to_wei(f'{c_obj.gas_price_gwei}', 'gwei')
            })
    ...
    @staticmethod
    def check_deployment_status(contract_obj: BlockchainSmartContract):
        ...
        try:
            ...
            try:
                tx_receipt = web3.eth.wait_for_transaction_receipt(
                    tx_hash, timeout=DEFAULT_TRANSACTION_RECEIPT_CHECK_TIMEOUT_SECONDS,
