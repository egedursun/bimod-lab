...
class PostgresSQLExecutor:
    ...
    def execute_read__headless(
        ...
        try:
            with psycopg2.connect(**headless_conn_params) as conn:
                with conn.cursor(
            ...
            logger.info(f"[Headless] Query executed successfully.")

        except Exception as e:
            output["status"] = False
            output["error"] = str(e)

            logger.error(f"[Headless] Error occurred while executing query: {e}")
        ...
        logger.info(f"[Headless] ]Transaction saved successfully.")
