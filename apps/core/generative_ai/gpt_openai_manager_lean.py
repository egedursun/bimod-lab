        ...
        try:
            ...
            try:
                system_prompt_msgs = [
                    SystemPromptFactoryBuilder.build_leanmod_system_prompts(
                        chat=self.chat, lean_assistant=self.lean_assistant, user=user, role=ChatRoles.SYSTEM
                    )
                ]
                transmit_websocket_log(f"""
            ...
            📈 Transaction parameters are being inspected...
                ...
                last_msg_cost = calculate_billable_cost_from_raw(
                    encoding_engine=GPT_DEFAULT_ENCODING_ENGINE, model=self.chat.lean_assistant.llm_model.model_name,
                    text=latest_message
                )
            except Exception as e:
            ...
            try:
                chunks = c.chat.completions.create(
                    model=self.lean_assistant.llm_model.model_name, messages=system_prompt_msgs,
                    ...
                    top_p=float(self.lean_assistant.llm_model.top_p), stream=True
                )
            except Exception as e:
        ...
        if tool_resp_list:
            ...
            try:
                tool_req = MultimodalLeanChatMessage.objects.create(
                    multimodal_lean_chat=self.chat, sender_type=ChatRoles.ASSISTANT.upper(),
            ...
            📦 Communication records for the tool responses are being prepared...
                ...
                tool_msg = MultimodalLeanChatMessage.objects.create(
                    multimodal_lean_chat=self.chat, sender_type=HistoryBuilder.ChatRoles.TOOL.upper(),
    ...
    def respond(self, latest_message, prev_tool_name=None, with_media=False, file_uris=None, image_uris=None):
        ...
        try:
            try:
                ...
                ext_msgs, encrypt_uuid = HistoryBuilder.build_leanmod(lean_chat=self.chat)
            ...
            try:
                final_output = c.chat.completions.create(
                    model=self.lean_assistant.llm_model.model_name, messages=prompt_msgs,
                    temperature=float(self.lean_assistant.llm_model.temperature),
                    frequency_penalty=float(self.lean_assistant.llm_model.frequency_penalty),
                    presence_penalty=float(self.lean_assistant.llm_model.presence_penalty),
                    max_tokens=int(self.lean_assistant.llm_model.maximum_tokens),
