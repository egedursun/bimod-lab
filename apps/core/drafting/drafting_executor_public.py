...
class DraftingExecutionManager_Public:
    ...
    def __init__(self, drafting_google_apps_connection: DraftingGoogleAppsConnection, text_content: str):
    ...
    def execute_select_command(self, selected_text: str, command: str):
        ...
        response = {'output': output, 'error': output}
        try:
            output, error = handle_select_command_public(xc=self, selected_text=selected_text, command=command, content=self.content)
            logger.info(
