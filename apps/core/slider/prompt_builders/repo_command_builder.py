...
def build_repo_command_system_prompt(xc: SliderExecutionManager, user_query: str):
    ...
    action_instructions_prompt = build_slider_action__repo_prompt(user_query=user_query)

    data_source_prompts = build_slider_code_base_data_source_prompt(assistant=xc.copilot)
    tool_execution_prompts = build_slider_tool_prompt__execute_codebase_query()
...
def build_repo_command_system_prompt_public(xc: SliderExecutionManager_Public, user_query: str, content: str):
    logger.info(f"Building REPO command system prompt for user query: {user_query}")
