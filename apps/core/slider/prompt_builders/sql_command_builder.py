...
def build_sql_command_system_prompt(xc: SliderExecutionManager, user_query: str):
    ...
    action_instructions_prompt = build_slider_action__sql_prompt(user_query=user_query)

    data_source_prompts = build_slider_sql_data_source_prompt(assistant=xc.copilot)
    tool_execution_prompts = build_slider_tool_prompt__execute_sql_query()
...
def build_sql_command_system_prompt_public(xc: SliderExecutionManager_Public, user_query: str, content: str):
    logger.info(f"Building SQL command system prompt for user query: {user_query}")
