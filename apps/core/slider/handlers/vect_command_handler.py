...
def handle_vect_command(xc, command: str) -> str:
    ...
    try:
        tx = LLMTransaction.objects.create(
            organization=xc.slider_document.organization,
            model=xc.copilot_llm,
            responsible_user=xc.slider_document.created_by_user,
    ...
    system_prompt = build_vect_command_system_prompt(
        xc=xc,
        user_query=command
    )
