...
class VideoGenerationExecutor:
    ...
    def _download_and_save_video(self, generation, video_url: str):
        ...
        try:
            tx = LLMTransaction(
                ...
                encoding_engine=GPT_DEFAULT_ENCODING_ENGINE, llm_cost=InternalServiceCosts.VideoGenerator.COST,
                transaction_type=ChatRoles.SYSTEM, transaction_source=LLMTransactionSourcesTypesNames.GENERATE_VIDEO,
                is_tool_cost=True)
            tx.save()
        ...
        save_name = generate_save_name(extension=file_format)
        bucket_path = f"{GENERATED_VIDEOS_ROOT_MEDIA_PATH}{save_name}"
        full_uri = f"{MEDIA_URL}{bucket_path}"
        try:
            s3c = boto3.client('s3')
            bucket = os.getenv('AWS_STORAGE_BUCKET_NAME')
            s3c.put_object(Bucket=bucket, Key=bucket_path, Body=data_bytes)
    ...
    def text_to_video(self, query: str, loop=False, with_aspect_ratio=False, aspect_ratio=None):
        ...
        while not completed:
            ...
            if generation.state == LumaAIGenerationStates.FAILED:
                ...
                return {"error": "The video generation process has failed on the client side.", "video_url": None}
    ...
    def text_and_image_to_video(self, query: str, start_frame_url: str = None, end_frame_url: str = None,
        ...
        if start_frame_url is None and end_frame_url is None:
            ...
            return {"error": "At least one of the start or end frame URLs must be provided.", "video_url": None}
        ...
        if frame_type == VideoGeneratorFrameTypes.START:
            ...
            generation = self.client.generations.create(
                prompt=query,
                keyframes={
                    "frame0": {"type": "image", "url": start_frame_url},
                    "frame1": {"type": "image", "url": end_frame_url}
                })
