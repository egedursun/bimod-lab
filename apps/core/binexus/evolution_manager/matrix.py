...
class Matrix:
    def __init__(self, binexus_process: BinexusProcess, population_size: int, mutation_rate_per_individual: float,
                 mutation_rate_per_gene: float, cross_over_rate: float, elitism_selection_ratio: float,
                 self_mating_possible: bool = False, maximum_epochs: int = 10):
        ...
        self.mutation_manager = MutationManager(mutation_chance_per_individual=mutation_rate_per_individual,
    ...
    def execute_evolution(self, is_test=False):
        try:
            latest_parents = self.population
            while self._current_epoch < self.maximum_epochs:
                latest_parents = self.iterate_epoch(is_test=is_test)

                tx = LLMTransaction(
                    organization=self.binexus_process.organization, model=self.binexus_process.llm_model,
                    responsible_user=self.binexus_process.created_by_user, responsible_assistant=None,
                    encoding_engine=GPT_DEFAULT_ENCODING_ENGINE,
                    llm_cost=InternalServiceCosts.Binexus.COST,
                    transaction_type=ChatRoles.SYSTEM,
                    transaction_source=LLMTransactionSourcesTypesNames.BINEXUS, is_tool_cost=True
                )
                tx.save()
    ...
    def iterate_epoch(self, is_test):
        print("Iterating New Generation")
        print("----------process: iterating new generation: start---------------")
        self._current_epoch += 1
        evaluated_population = self.fitness_manager.evaluate_and_record_population_fitness(population=self.population,
                                                                                           is_test=is_test)
        self.population = evaluated_population
        self.population_manager.build_hall_of_fame(population=self.population)
        children = self.breeding_manager.produce_new_generation(population=self.population)
        mutated_children = self.mutation_manager.mutate_population(population=children)
        ...
        print("- Iterated new generation: ", self._current_epoch)
        print("-----------process: iterating new generation: end--------------")
    ...
    def save_visual_records(self):
        print("Saving Visual Records")
        print("----------process: saving visual records: start-----------")
        plt.style.use('dark_background')
        plt.title('Evolutionary Optimization Process History', color='white')
        plt.plot(self.average_fitness_per_epoch, label='Average Fitness per Generation', color='yellow')
        plt.plot(self.best_fitness_per_epoch, label='Best Fitness per Generation', color='green')
        plt.plot(self.worst_fitness_per_epoch, label='Worst Fitness per Generation', color='red')
        plt.xlabel('Generations', color='white')
        plt.ylabel('Fitness Level (Absolute)', color='white')
        ...
        try:
            with open(os.path.join(tmp_path, f'process_optimization_chart_{randomized_filename_component}.png'),'rb') as f:
                self.process_history_chart = f.read()
        except Exception as e:
            logger.error(f"Error occurred while converting the image file into bytes: {e}")
        try:
            os.remove(os.path.join(tmp_path, f'process_optimization_chart_{randomized_filename_component}.png'))
        except Exception as e:
            logger.error(f"Error occurred while deleting the image file from temporary folder: {e}")

        print("- Visual Records Saved")
        print("---------process: saving visual records: end------------")

    def flush(self):
        print("Flushing Evolution Manager")
        print("-----------process: flushing evolution manager: start--------------")
        self._current_epoch = 0
        self.population = self.population_manager.build_population_with_size()
        print("- Flushed Evolution Manager")
        print("-----------process: flushing evolution manager: end--------------")
