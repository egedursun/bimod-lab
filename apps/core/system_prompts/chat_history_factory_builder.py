...
class HistoryBuilder:
    ...
    def build_chat_history(chat: MultimodalChat, ner_executor: NERExecutor = None):
        ...
        for msg in msgs:
            ...
            msg_obj = {"role": src_type.lower()}
            wrapper = [{"type": "text", "text": message_text_content}]
            ...
            if src_type != HistoryBuilder.ChatRoles.SYSTEM:
                ...
                if ner_executor:
                    ...
                    if encrypted_text:
                        wrapper = [{"type": "text", "text": encrypted_text}]
                        msg_obj["content"] = wrapper
                ####################################################################################################
                ####################################################################################################
            ...
            tx = LLMTransaction.objects.create(
                organization=chat.organization, model=chat.assistant.llm_model, responsible_user=chat.user,
                responsible_assistant=chat.assistant, encoding_engine=GPT_DEFAULT_ENCODING_ENGINE,
    ...
    def build_leanmod(lean_chat: MultimodalLeanChat):
        ...
        for msg in msgs:
            ...
            msg_obj = {"role": source_type.lower()}
            wrapper = [{"type": "text", "text": txt_content}]
            ...
            if f_urls and source_type == HistoryBuilder.ChatRoles.USER:
                for f_url in f_urls:
                    f_uri_info_wrapper = {"type": "text", "text": f"Detected File URLs: {f_url}"}
            ...
            tx = LLMTransaction.objects.create(
                organization=lean_chat.organization, model=lean_chat.lean_assistant.llm_model,
                responsible_user=lean_chat.user, responsible_assistant=None,
                encoding_engine=GPT_DEFAULT_ENCODING_ENGINE, transaction_context_content=txt_content,
    ...
    def build_voidforger(voidforger_chat: MultimodalVoidForgerChat):
        ...
        for msg in msgs:
            ...
            tx = LLMTransaction.objects.create(
                ...
                responsible_user=voidforger_chat.user, responsible_assistant=None,
                encoding_engine=GPT_DEFAULT_ENCODING_ENGINE, transaction_context_content=txt_content,
                llm_cost=0, internal_service_cost=0, tax_cost=0, total_cost=0, total_billable_cost=0,
                transaction_type=source_type.lower(), transaction_source=voidforger_chat.chat_source)
