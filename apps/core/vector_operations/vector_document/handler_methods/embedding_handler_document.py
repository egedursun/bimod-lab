...
def embed_document_sync(
    ...
    connection_orm_object = DocumentKnowledgeBaseConnection.objects.get(
        id=connection_id
    )
    ...
    try:
        collection = c.collections.get(
            executor.connection_object.class_name
        )
...
def embed_document_helper(
    ...
    try:
        ...
        if error:
            logger.error(
                f"[document_embedder.embed_document_helper] Error building the document ORM structure: {error}")
        ...
        if error:
            logger.error(
                f"[document_embedder.embed_document_helper] Error building the document Weaviate structure: {error}")
