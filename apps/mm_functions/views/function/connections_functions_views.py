...
class CustomFunctionView_Connections(LoginRequiredMixin, TemplateView):
    def get_context_data(self, **kwargs):
        ...
        ext_function_refs = CustomFunctionReference.objects.filter(
        ...
        ext_function_refs_map = {
            assistant.id: set(
                reference for reference in ext_function_refs if reference.assistant.id == assistant.id
            )
            for assistant in agents
        }
        ...
        context.update(
            {
                ...
                'functions': functions,
                'assistants': agents,
                'assistant_function_map': agent_function_map,
                'external_function_references_map': ext_function_refs_map
            }
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if action == 'add' and function_id:
                ...
                if ref_id:
                    reff = CustomFunctionReference.objects.get(
                        id=ref_id
                    )
