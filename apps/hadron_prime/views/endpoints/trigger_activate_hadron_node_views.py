        ...
        try:
            ...
            if not api_key:
                return JsonResponse(
                    {
                        "success": False,
                        "error": "API key is missing from the request. Make sure to "
            ...
            xc_log = HadronNodeExecutionLog.objects.create(
                node=node,
                execution_log=None,
                execution_status=HadronNodeExecutionStatusesNames.PENDING,
            ...
            xc_log.execution_log += NodeExecutionProcessLogTexts.system_initialized(
                node=node
            )

            xc_log.execution_log += NodeExecutionProcessLogTexts.topics_initialized(
                node=node
            )

            xc_log.execution_log += NodeExecutionProcessLogTexts.node_initialized(
                node=node
            )
            ...
            if error:
                ...
                xc_log.execution_log += NodeExecutionProcessLogTexts.process_failed(
                    error_log=error
                )
