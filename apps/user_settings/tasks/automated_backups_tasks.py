        ...
        for backup_type in backup_types:
            ...
            try:
                if backup_type == BackupTypesNames.LLM_MODELS:
                    ...
                    executor = DataBackupExecutor.BackupLLMModel(
                        organization=org,
                        responsible_user=org.users.first()
                    )

                    executor.backup_llm_models(
                        backup_name=backup_name,
                        password=sample_number
                    )
                    ...
                    executor = DataBackupExecutor.BackupNERInstance(
                        organization=org,
                        responsible_user=org.users.first()
                    )

                    executor.backup_ner_instances(
                        backup_name=backup_name,
                        password=sample_number
                    )
                    ...
                    executor = DataBackupExecutor.BackupAssistant(
                        organization=org,
                        responsible_user=org.users.first()
                    )

                    executor.backup_assistants(
                        backup_name=backup_name,
                        password=sample_number
                    )
                    ...
                    executor = DataBackupExecutor.BackupCustomFunction(
                        organization=org,
                        responsible_user=org.users.first()
                    )

                    executor.backup_custom_functions(
                        backup_name=backup_name,
                        password=sample_number
                    )
                    ...
                    executor = DataBackupExecutor.BackupCustomAPI(
                        organization=org,
                        responsible_user=org.users.first()
                    )

                    executor.backup_custom_apis(
                        backup_name=backup_name,
                        password=sample_number
                    )
                    ...
                    executor = DataBackupExecutor.BackupCustomScript(
                        organization=org,
                        responsible_user=org.users.first()
                    )

                    executor.backup_custom_scripts(
                        backup_name=backup_name,
                        password=sample_number
                    )
