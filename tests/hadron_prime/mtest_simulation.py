...
ROBOT_ACTION_SPACE_X = [
    x for x in range(
        -ROBOT_MAX_STEP_SIZE_X,
        ROBOT_MAX_STEP_SIZE_X + 1
    )
]

ROBOT_ACTION_SPACE_Y = [
    y for y in range(
        -ROBOT_MAX_STEP_SIZE_Y,
        ROBOT_MAX_STEP_SIZE_Y + 1
    )
]
...
class RobotAgent:
    ...
    def act(
        ...
        if (
            move_x not in ROBOT_ACTION_SPACE_X or
            move_y not in ROBOT_ACTION_SPACE_Y
        ):
            raise ValueError("move_x and move_y must be in the action space.")
...
def visualize_environment(
    ...
    grid = [
        row[:] for row in cur_environment.grid
    ]
...
@app.get("/goal_state")
def goal_state():
    nearest_food = robot.find_nearest_food(
        env.grid
    )

    if nearest_food is None:
        return {
            "goal": "No food remaining on the grid."
        }

    return {
        "goal_position_x": nearest_food[0],
        "goal_position_y": nearest_food[1]
    }
...
def error_calculation():
    ...
    if nearest_food is None:
        return {
            "error": "No food remaining on the grid"
        }
